...
if [[ -f install.sh ]]; then
    chmod +x install.sh
    # Run install.sh exactly once. Only use sudo if it's actually available
    # and we're not already root — the old "sudo X || X" pattern would
    # silently re-run the ENTIRE installer a second time (duplicating the
    # panel process, redoing apt installs, etc.) whenever the first run
    # failed partway through for any reason, not just "no sudo".
    if [[ "$(id -u)" -ne 0 ]] && command -v sudo >/dev/null 2>&1; then
        sudo -E AKVM_DOMAIN="$AKVM_DOMAIN" bash install.sh
    else
        AKVM_DOMAIN="$AKVM_DOMAIN" bash install.sh
    fi
else
